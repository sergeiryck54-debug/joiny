    ...
    (async () => {
      try {
        ...
        if (status === 'granted') {
          ...
          setLocation({ lat: 12.9236, lng: 100.8825 });
        }
      } catch (e) {
        setLocation({ lat: 12.9236, lng: 100.8825 });
      }
      try {
        ...
        if (data) setDbEvents(data.map((e: any) => ({
          id: e.id, title: e.title, category: e.emoji, lat: e.lat, lng: e.lng,
          people: e.people, max: e.max_people, now: e.is_now,
        })));
      } catch (e) {}
      setLoading(false);
    })();
        ...
        {filtered.map(event => (
          <View key={event.id} style={styles.card}>
            ...
            <TouchableOpacity
              ...
              onPress={() => setJoined(prev =>
                prev.includes(event.id) ? prev.filter(i => i !== event.id) : [...prev, event.id]
              )}
            >
...
const styles = StyleSheet.create({
  ...
  joinBtn: { backgroundColor: '#111', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 10 },
